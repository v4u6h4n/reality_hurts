...
def get_client_id():
    client_id_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'client_id.txt')
...
def get_client_secret():
    client_secret_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'client_secret.txt')
...
class Bot(commands.Bot):
    ...
    def __init__(self):
        super().__init__(token=self.get_access_token(), prefix='!', initial_channels=['channel'])
        self.setup_logging()
        self.global_emotes = None


    def get_access_token(self):
        access_token_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'access_token.txt')
                ...
                for pos in positions:
                    ...
                    if emote_text in self.global_emotes:
                        ...
                        emote_rendering_process = subprocess.run(
                            ['python3', 'send_png.py', emote_image_url],
                            capture_output=True,
                            text=True
                        )

                        # Strip any leading/trailing whitespace, including line breaks
                        rendered_emote = emote_rendering_process.stdout.strip()

                        # Now replace the emote text in the modified_message
