...
prerequisite() {
    ...
    prerequisite_device() {
        ...
        # Output.
            ...
            output_device_headphones_1_node_name="bluez_output.74_2A_8A_40_AD_0E.1" # "bluez_output.94_DB_56_03_17_D5.1" "$(yq -r ".device.audio_output.headset.node" "$path_settings") # "bluez_output.89_88_1F_FD_47_F7.1" # 
            output_device_headphones_1_address="74:2A:8A:40:AD:0E" # "94:DB:56:03:17:D5" $(yq -r ".device.audio_output.headset.address" "$path_settings") # "89:88:1F:FD:47:F7" # headset: 
...
command() {
    ...
    command_system() {
        ...
        if [[ "$arg_system" == "application" ]]; then
            arg_system_application="$2"
            arg_system_application_profile="$3"
            arg_system_application_action="$4"
        
            setting_update system

        # Window manager.
        elif [[ "$arg_system" == "window_manager" ]]; then
            
            setting_update_system "${@:2}"
        
        # Error.
        else
            echo_error "command_system, arg_system: $arg_system."
        fi
...
translate_argument() {
    ...
    echo_debug "Translate argument: $1"
    ...
    # System.
        ...
        if [[ "$2" == "obs_studio" || "$2" == "o" ]]; then
            argument="obs_studio"
        elif [[ "$2" == "window_manager" ]]; then
            argument="window_manager"
        else
            echo_error "translate_argument, system, invalid argument: ${2}."
        fi

    # System profile.
    elif [[ "$1" == "system_profile" ]]; then
        ...
        if [[ "$2" == "restricted" ]]; then
            ...
            echo_error "translate_argument, system_profile, invalid argument: ${2}."
        fi
    ...
    # Command.
        ...
        if [[ "$2" == "--activity" || "$2" == "activity" || "$2" == "-a" || "$2" == "a" ]]; then
            argument="activity"
        elif [[ "$2" == "--system" || "$2" == "system" || "$2" == "-ap" || "$2" == "ap" ]]; then
            argument="system"
    ...
    setting_update_system() {
        ...
        setting_update_system_loopback_start() {
            ...
            setting_update_system_loopback_start_bed_overhead_obs() {
                ...
                v4l2-ctl -d $path_camera_bed_overhead -c focus_automatic_continuous=0
                ffmpeg -hwaccel vaapi -f v4l2 -framerate 60 -video_size 1920x1080 -input_format mjpeg -i $path_camera_bed_overhead -pix_fmt yuv420p -f v4l2 /dev/video70 > /dev/null 2>&1 & disown
            ...
            setting_update_system_loopback_start_bed_tripod_obs() {
                ...
                v4l2-ctl -d $path_camera_bed_tripod -c focus_automatic_continuous=0
                ffmpeg -hwaccel vaapi -f v4l2 -framerate 60 -video_size 1920x1080 -input_format mjpeg -i $path_camera_bed_tripod -pix_fmt yuv420p -f v4l2 /dev/video60 > /dev/null 2>&1 & disown
        ...
        setting_update_system_camera_bed_tripod_start() {
            ...
            mpv av://v4l2:/dev/video61 --no-cache --no-input-cursor --untimed --osc=no --stop-screensaver=no --panscan=1 --profile=low-latency --no-cache --no-config --title="camera_bed_tripod" & disown
